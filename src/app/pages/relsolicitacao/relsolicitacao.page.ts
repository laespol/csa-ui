...
@Component({
  selector: 'app-relsolicitacao',
  templateUrl: './relsolicitacao.page.html',
  styleUrls: ['./relsolicitacao.page.scss'],
})
export class RelsolicitacaoPage implements OnInit {
  ...
  async inicio() {
    ...
    //    if (this.solicitacaoService.posicao != null) {
    this.posicao = this.contratoService.posicao;
    this.dt22 = this.contratoService.dt22;
    //  this.idimovel = this.contratoService.idimovel;

    await this.consultaTpsolicitacao();
    // await this.consultaCusto();
  ...
  async consultaColuna() {
    ...
    this.coluna.programa = 'solicitacao';
    ...
    if (this.usuariocolunas != null || this.usuariocolunas != undefined) {
      ...
      for (let i = 0; i < this.usuariocolunas.length; i++) {
        ...
        this.selectedColumns.push(this.usuariocolunas[i].coluna);
      }
  ...
  async save() {
    console.log(this.coluna.programa);
  ...
  async LoadItcompra(event: LazyLoadEvent) {
    ...
    this.posicao = event;
    this.contratoService.posicao = event;

    if (event.sortField == undefined) {
      event.sortField = 'idsolicitacao';
      event.sortOrder = 0;
    }
    if (event.globalFilter == undefined) {
      if (this.contratoService.dt22 != '') {
    ...
    //    if (event.filters == undefined || event.filters == null || this.idimovel == '0') {
    //      this.idimovel = '0';
    ...
    /*     for (let prop in event.filters) {
           ...
           let filterMeta = event.filters[filterField];
           this.idimovel = JSON.stringify(filterMeta[0].value.idimovel);
           console.log("filterMeta " + JSON.stringify(filterMeta[0].value.idimovel) + " - " + this.idimovel);
   */
    //      }
    //    }
    ...
    await this.solicitacaoService.consultaSolicitacaos(event.first, event.rows, event.sortField, event.sortOrder, event.globalFilter, this.idimovel);
    this.solicitacaos = this.solicitacaoService.solicitacaos;
    //      console.log (" CONTRATOS = " + JSON.stringify(this.solicitacaos));
    this.totalRecords = this.solicitacaoService.totalRecords;
    this.loading = false;
  ...
  async exclui(event) {
    ...
    this.solicitacao = event;
    await this.solicitacaoService.apagaSolicitacao(this.solicitacao);
  ...
  async finalizar(event) {
    ...
    //    await this.solicitacaoService.atualizaSolicitacao(this.solicitacao);
  ...
  async novo() {
    //   this.solicitacaoService.solicitacao = null;
    this.contratoService.dt22 = this.dt22;

    this.contratoService.posicao = this.posicao;

    this.solicitacao.idsolicitacao = 0;
    this.solicitacao.email = '';
    this.solicitacao.descricao = '';
    this.solicitacao.nome = '';
    this.solicitacao.telefone = '';
    this.solicitacao.sigilo = null;
    this.solicitacao.status = '';
    this.solicitacao.statussolicitacao = '';
    this.solicitacao.idtpsolicitacao = null;
    this.solicitacao.idusercreateAt = null;
    this.solicitacao.createdAt = null;
    this.solicitacao.iduserupdatedAt = null;
    this.solicitacao.updatedAt = null;

    console.log("this.solicitacao = " + this.solicitacao);

    this.solicitacaoService.solicitacao = this.solicitacao;

    console.log("this.solicitacaoService.solicitacao" + this.solicitacaoService.solicitacao);

    this.navCtrl.navigateRoot('menu/solicitacaonovo', { animated: true });
  }

  async onRowSelect(event) {
    this.solicitacao = event.data;
    //    console.log("this.solicitacao.custo.idimovel" + this.solicitacao.custo.idimovel)
    this.solicitacaoService.solicitacao = this.solicitacao;
    this.contratoService.dt22 = this.dt22;

    this.contratoService.posicao = this.posicao;
    //    console.log("this.solicitacaoService.posicao" + JSON.stringify(this.solicitacaoService.posicao));
    this.navCtrl.navigateRoot('menu/solicitacaonovo', { animated: true });
  }
  ...
  async download(event) {
    ...
    await this.documentoService.consultaDocumentoDetail(this.solicitacao);

    this.documento = this.documentoService.documento;

    console.log(this.documento.documentopdf);

    //    var blob = new Blob([this.documento.documentopdf], { type: this.documento.mimetype });
    //   saveAs(blob, this.solicitacao.idsolicitacao + '-' + this.solicitacao.razaosocial + ".pdf");
    ...
    //   this.documento.documentopdf.arrayBuffer;

    //FileSaver.saveAs(linkSource, this.solicitacao.idsolicitacao + '-' + this.solicitacao.razaosocial + '.pdf');



    //let url = window.URL.createObjectURL(JSON.stringify(this.documento.documentopdf));
    //  let a = document.createElement('a');
    // a.href = JSON.stringify(this.documento.documentopdf);
    // a.download = 'teste.pdf';
    //  a.click();



    // var blob : Blob = this.documento.documentopdf

    //let res = await fetch(`data:${this.documento.mimetype};base64,${JSON.stringify(this.documento.documentopdf)}`);
    ...
    // const fileName = this.solicitacao.idsolicitacao + '-' + this.solicitacao.razaosocial + '.pdf';
    ...
    // console.log("Blob = " + linkSource);

    // console.log("buff = " + this.documento.documentopdf.toString); // 10

    //  const linkSource = Buffer.from(this.documento.documentopdf, 'hex');

    //const linkSource = this.documento.documentopdf.;
    //const downloadLink = document.createElement('a');
    //const fileName = this.solicitacao.idsolicitacao + '-' + this.solicitacao.razaosocial + '.pdf';

    //downloadLink.href = this.documento.documentopdf;
    // downloadLink.download = fileName;
    //downloadLink.click();

    //await this.createBlob(this.documento.documentopdf);

    // const arrUint8 = new Uint8Array(JSON.stringify(this.documento.documentopdf));

    //  const blob = this.documento.documentopdf;
    //  console.log("Blob = " + blob + " JSON = " + JSON.stringify(blob))
    ...
    // downloadLink.download = this.solicitacao.idsolicitacao + '-' + this.solicitacao.razaosocial + '.pdf';
    // downloadLink.click();
    // downloadLink.remove();
    //   var blob = new Blob([this.documento.documentopdf], { type: this.documento.mimetype});
    ...
    //   var blob = new Blob([this.documento.documentopdf], { type: "application/pdf;charset=utf-8" });
  ...
  async exportExcel(event: LazyLoadEvent) {
    ...
    await this.solicitacaoService.consultaSolicitacaos(event.first, event.rows, event.sortField, event.sortOrder, event.globalFilter, this.idimovel);
    //  await this.solicitacaoService.getSolicitacaogeral();
    this.solicitacaos = this.solicitacaoService.solicitacaos;
    ...
    for (let i = 0; i < this.solicitacaos.length; i++) {
      this.linha = [];
      for (let x = 0; x < this.selectedColumns.length; x++) {
        if (this.selectedColumns[x].type == 'string') {
          this.linha[x] = this.solicitacaos[i][this.selectedColumns[x].field];
        }
        if (this.selectedColumns[x].type == 'soma') {
          this.linha[x] = this.solicitacaos[i][this.selectedColumns[x].agregado2].replace('.', ',');
        }
        if (this.selectedColumns[x].type == 'date') {
          if (this.solicitacaos[i][this.selectedColumns[x].field] == null) {
            this.dtviginicio = '';
          } else {
            this.dtviginicio = new Date(this.solicitacaos[i][this.selectedColumns[x].field]).toLocaleDateString();
          }
        ...
        if (this.selectedColumns[x].type == 'agregate1') {
          this.linha[x] = this.solicitacaos[i][this.selectedColumns[x].field][this.selectedColumns[x].agregado1];
        }
        if (this.selectedColumns[x].type == 'agregate2') {
          this.linha[x] = this.solicitacaos[i][this.selectedColumns[x].field][this.selectedColumns[x].agregado1][this.selectedColumns[x].agregado2];
        }
        if (this.selectedColumns[x].type == 'agregate3') {
          this.linha[x] = this.solicitacaos[i][this.selectedColumns[x].field][this.selectedColumns[x].agregado1];
        }
        if (this.selectedColumns[x].type == 'status') {
          if (this.solicitacaos[i][this.selectedColumns[x].field]) {
            this.linha[x] = 'Sim';
          } else {
            this.linha[x] = 'Não';
          }
        }
        if (this.selectedColumns[x].type == 'stsolicitacao') {
          if (this.solicitacaos[i][this.selectedColumns[x].field]) {
            this.linha[x] = 'Ativo';
          } else {
    ...
    import("xlsx").then(xlsx => {
      ...
      xlsx.utils.book_append_sheet(workbook, worksheet, 'Solicitacaos');
      const excelBuffer: any = xlsx.write(workbook, { bookType: 'xlsx', type: 'array' });
      this.saveAsExcelFile(excelBuffer, `solicitacaos`);
    });
  ...
  async gerarPdf(event: LazyLoadEvent) {
    await this.uiService.present({
      message: 'aguarde'
    });
    ...
    this.nomearquivo = 'Solicitacao_' + new Date().toLocaleDateString() + '.pdf';
    ...
    for (let i = 0; i < this.solicitacaos.length; i++) {
      ...
      //     await this.usuarioService.consultaGestor(this.solicitacaos[i].unidade);

      //     this.gestor = this.usuarioService.gestor;

      if (this.solicitacaos[i].statussolicitacao == 'A') {
        this.status = 'Ativo'
      } else {
        this.status = 'Finalizado'
      };

      if (this.solicitacaos[i].sigilo == 0) {
        this.sigilo = 'Normal';
      } else {
        if (this.solicitacaos[i].sigilo == 1) {
          this.sigilo = 'Sigiloso';
        } else {
          this.sigilo = 'Anonimo';
        }
      }
      ...
      /*     if (this.solicitacaos[i].tipopagamento == '') {
             this.tipopagamento = '';
           } else {
             this.tipopagamento = this.solicitacaos[i].tipopagamento;
             if (this.solicitacaos[i].numeroparcelas != 0) {
               this.tipopagamento =  this.tipopagamento + " Numero de parcelas : " + this.solicitacaos[i].numeroparcelas;
             }
           };
           */
      console.log('solicitacao = ' + JSON.stringify(this.solicitacaos[i]));
      this.lista.push({
        idsolicitacao: this.solicitacaos[i].idsolicitacao,
        sigilo: this.sigilo,
        nome: this.solicitacaos[i].nome,
        email: this.solicitacaos[i].email,
        telefone: this.solicitacaos[i].telefone,
        descricao: this.solicitacaos[i].descricao,
        resposta: this.solicitacaos[i].resposta,
        datacriacao: new Date(this.solicitacaos[i].createdAt).toLocaleDateString(),
        tpsolicitacao: this.solicitacaos[i].tpsolicitacao.descricao,
        status: this.status
    ...
    //      console.log("imovel - " + this.hora.unidade.idimovel);

    //   if (this.solicitacao.unidade.idimovel == 4) {
    imagem1 = mosteiro;
    ...
    var docDefinition = {
      ...
      footer: function (currentPage, pageCount) {
        return [
          { text: currentPage.toString() + ' de ' + pageCount, alignment: 'center', fontSize: 10 },
        ]
      },
          ...
          body: [
            ...
            },
            {
              ...
              table: {
                widths: ['*'],
                body: [
                  [{ text: 'Relatorio de Solicitações para Ouvidoria', bold: true, fontSize: 16, alignment: 'center', border: [false, false, false, false] }],
                  [{ text: new Date().toLocaleDateString(), bold: true, fontSize: 16, alignment: 'center', border: [false, false, false, false] }]
                ]
              },
            },
            {
      ...
      content: [
...

        {
          ...
          table: {
            headerRows: 1,
            widths: [50, 40, 50, 60, 60, 150, 150, 50, 50, '*'],
            body: [
              [
                { text: 'Identificador', fontSize: 10, bold: true, alignment: 'center', fillColor: '#dddddd' },
                { text: 'Sigilo', fontSize: 10, bold: true, alignment: 'center', fillColor: '#dddddd' },
                { text: 'Nome', fontSize: 10, bold: true, alignment: 'center', fillColor: '#dddddd' },
                { text: 'E-mail', fontSize: 10, bold: true, alignment: 'center', fillColor: '#dddddd' },
                { text: 'Telefone', fontSize: 10, bold: true, alignment: 'center', fillColor: '#dddddd' },
                { text: 'Descrição', fontSize: 10, bold: true, alignment: 'center', fillColor: '#dddddd' },
                { text: 'Resposta', fontSize: 10, bold: true, alignment: 'center', fillColor: '#dddddd' },
                { text: 'Tipo Solicitação', fontSize: 10, bold: true, alignment: 'center', fillColor: '#dddddd' },
                { text: 'Data', fontSize: 10, bold: true, alignment: 'center', fillColor: '#dddddd' },
                { text: 'Status', fontSize: 10, bold: true, alignment: 'center', fillColor: '#dddddd' },
              ],
              ...this.lista.map(p =>
              (

                [
                  { text: p.idsolicitacao, fontSize: 8, alignment: 'center' },
                  { text: p.sigilo, fontSize: 8, alignment: 'center' },
                  { text: p.nome, fontSize: 8, alignment: 'center' },
                  { text: p.email, fontSize: 8, alignment: 'center' },
                  { text: p.telefone, fontSize: 8, alignment: 'center' },
                  { text: p.descricao, fontSize: 8, alignment: 'center' },
                  { text: p.resposta, fontSize: 8, alignment: 'center' },
                  { text: p.tpsolicitacao, fontSize: 8, alignment: 'center' },
                  { text: p.datacriacao, fontSize: 8, alignment: 'center' },
                  { text: p.status, fontSize: 8, alignment: 'center' },
                ]
              )
    ...
    pdfmake.createPdf(docDefinition).download(this.nomearquivo);
